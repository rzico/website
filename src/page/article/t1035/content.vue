  ...
  <div class="mp-content">
    <div  v-for="(template,index) in templatesList"  v-if="isShow(index)">
      <div class="margin-section section section-on section-border text-up fill">
        ...
        <!--判断类型是图文还是小视频-->
        <div class="img-box  offline-preview " v-if="hasImage(template,index) && index % 2 == 0" >
          <div class="eq1 img-bg-1"><div class="news1 blue-news"></div> <div class="square square-red"></div></div>
          <div class="eq2"></div>
          <div class="logo1 bian-1"></div>
          <img
            v-lazy="template.original "
            class="images  preview-img img-padding-1"@click="imgPreview(template.original,templatesList.previewList)" ref="imgRef"/>
        </div>
        <!--判断类型是图文还是小视频-->
        <div class="img-box  offline-preview " v-if="hasImage(template,index) && index % 2 != 0" >
          <div class="eq1 img-bg-2"><div class="news1 red-news1"></div> <div class="square square-blue1"></div></div>
          <div class="eq2 che1header"></div>
          <div class="logo1 bian-2"></div>
          <img
            v-lazy="template.original "
            class="images  preview-img img-padding-2"@click="imgPreview(template.original,templatesList.previewList)" ref="imgRef"/>
        </div>
        <!--判断类型是否小视频-->
